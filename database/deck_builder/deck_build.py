...
library = {k.lower(): v for k,v in card_library().items()}
commander = input('What is the name of your Commander for this deck? ')
...
def deck_build():
    ...
    while building:
        ...
        if addition.lower() in library.keys():
            ...
            img_name = card['name'].replace(' ','-').replace(',','').replace('"','') +'.jpg'
            ...
            if card['name_back'] != 'N/B':
                back_img = card['name'].replace(' ','-').replace(',','').replace('"','')+"back.jpg"
            ...
            if len(good) >= 20:
                ...
                print("Try Again.")
                os.remove(img_name)
        count = countOccurance(deck,card['name'])
        if count >= 2:
...
def deck_manager():
    ...
    try:
        ...
        for card in cards:
            ...
            countOccurance(deck,magic_card)
        conn.close()
